// Konverto task-at në fatura duke përdorur Nexus Brain AI
app.post('/api/tasks/convert-to-invoice', authenticateToken, async (req, res) => {
  try {
    ...
    // Përgatit të dhënat për Nexus Brain
    const systemPrompt = `You are Nexus Brain, an expert in business automation. Your job is to convert a list of project tasks into a professional invoice.\n\nOBJECTIVES:\n- Identify billable items from the tasks.\n- For each billable item, extract: description, quantity (hours or units), unit price (if available), and total.\n- Suggest a professional invoice structure in JSON.\n- If any task is not billable, ignore it.\n- Add a summary with subtotal, taxes (if any), and grand total.\n\nReturn ONLY JSON in this format:\n{\n  \'items\': [\n    {\n      \'description\': string,\n      \'quantity\': number,\n      \'unit_price\': number,\n      \'total\': number\n    }\n  ],\n  \'summary\': {\n    \'subtotal\': number,\n    \'tax\': number,\n    \'grand_total\': number\n  }\n}`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Here are the tasks in JSON format:\n${JSON.stringify(tasks)}` }
        ],
        temperature: 0.1,
    ...
    if (!response.ok) throw new Error('Nexus Brain Invoice Conversion Failed');
    const data = await response.json();
    const invoice = JSON.parse(data.choices[0].message.content);
    res.json(invoice);
  } catch (err) {
    console.error('[Convert Tasks to Invoice Error]', err);
