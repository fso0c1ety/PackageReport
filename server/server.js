    ...
    const result = await db.query(`
          ...
          (
            ...
            FROM (
                ...
                jsonb_array_elements(
                  CASE
                    WHEN jsonb_typeof(COALESCE(t2.shared_users, '[]'::jsonb)) = 'array'
                    THEN COALESCE(t2.shared_users, '[]'::jsonb)
                    ELSE '[]'::jsonb
                  END
                ) elem
      ...
      WHERE w.owner_id = $1 OR EXISTS (
        ...
        FROM jsonb_array_elements(
          CASE
            WHEN jsonb_typeof(COALESCE(t.shared_users, '[]'::jsonb)) = 'array'
            THEN COALESCE(t.shared_users, '[]'::jsonb)
            ELSE '[]'::jsonb
          END
        ) AS elem
        WHERE elem->>'userId' = $1
...
app.post('/api/workspaces', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      ...
      [defaultTableId, `${newWorkspace.name} Table`, newWorkspace.id, JSON.stringify(columns), new Date().toISOString(), inviteCode]
    );
...
app.post('/api/tables/join', authenticateToken, async (req, res) => {
  try {
    ...
    if (!sharedUsers.some(u => u.userId === req.user.id)) {
      ...
      const seatCheck = await billingService.assertSeatAvailable(ownerId, req.user.id);
      if (!seatCheck.allowed) {
        return res.status(402).json({ error: 'Plan seat limit reached', code: 'SEAT_LIMIT_REACHED', billing: seatCheck.status });
      }
