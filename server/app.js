...
function mountCoreRoutes(app, {
  ...
  for (const route of [routes.users, routes.nexus, routes.uploads, routes.pushNotifications, routes.people, routes.automation, routes.emailer, routes.friends, routes.chats]) {
    app.use("/api", route);
  }
  return app;
}
