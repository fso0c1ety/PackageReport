...
function mountCoreRoutes(app, {
  ...
  for (const route of [routes.users, routes.workspaces, routes.tableMetadata, routes.nexus, routes.uploads, routes.pushNotifications, routes.notifications, routes.tableCollaboration, routes.people, routes.automation, routes.emailer, routes.friends, routes.chats]) {
    app.use("/api", route);
  }
  return app;
}
