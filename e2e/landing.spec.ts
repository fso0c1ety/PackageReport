...
test("canonical public navigation remains usable on mobile", async ({ page }) => {
  ...
  if (await openNavigation.isVisible()) {
    ...
    await expect(mobileNavigation.getByText(/Product/i)).toBeVisible();
    await expect(mobileNavigation.getByText(/Request Demo/i)).toBeVisible();
    await expect(mobileNavigation.getByRole("link", { name: /Download/i })).toHaveAttribute("href", /Smart\.Manage\.zip$/);
  }
