...
import { Alert, Box, Typography, Avatar, TextField, IconButton, Paper, List, ListItem, ListItemButton, useTheme, alpha, CircularProgress, Button, Badge, Tooltip } from "@mui/material";
            ...
            <Box sx={{
                ...
                <IconButton
                    ...
                    sx={{
                        mb: 0.5,
                        mr: 0.5,
                        bgcolor: draft.trim() ? '#6366f1' : alpha(theme.palette.text.primary, 0.05),
                        color: draft.trim() ? '#fff' : theme.palette.text.disabled,
                        transition: 'background-color 0.3s, color 0.3s, transform 0.3s',
                        transform: draft.trim() ? 'scale(1)' : 'scale(0.95)',
                        '&:hover': {
                            bgcolor: draft.trim() ? '#4f46e5' : alpha(theme.palette.text.primary, 0.08),
                            transform: draft.trim() ? 'scale(1.05)' : 'scale(0.95)'
                        },
                        width: 40,
                        height: 40
                    }}
                >
                    {sending ? <CircularProgress size={18} color="inherit" /> : <SendIcon sx={{ fontSize: 20, ml: draft.trim() ? 0.5 : 0 }} />}
                </IconButton>
            </Box>
        </Box>
    );
...
function ChatContent() {
    ...
    const handleSendMessage = React.useCallback(async (messageText: string) => {
        ...
        if (!trimmedMessage || !otherUserId) return false;
        setChatError("");
        ...
        try {
            ...
            if (!res.ok) throw new Error(responseData?.error || `Message could not be sent (${res.status})`);

            setMessages(prev => prev.map(item => item.id === optimisticMessage.id ? responseData : item));
                ...
                <Box sx={{
                    ...
                    {!otherUserId ? (
                        ...
                        <>
                            ...
                            {chatError && <Alert severity="error" onClose={() => setChatError("")}>{chatError}</Alert>}
