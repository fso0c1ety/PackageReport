...
export default function MarketplacePage() {
  ...
  const saveReview = async () => {
    ...
    try {
      const response = await authenticatedFetch(getApiUrl(`marketplace/${reviewing.id}/reviews`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(reviewDraft) });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || "Unable to save review");
      showNotification("Review saved successfully", "success"); setReviewing(null); setReviewDraft({ rating: 5, review: "" }); await loadCommunity();
    } catch (error) { showNotification(error instanceof Error ? error.message : "Unable to save review", "error"); }
  ...
  return <Box sx={{ maxWidth: 1240, mx: "auto", px: { xs: 2, md: 4 }, py: { xs: 3, md: 5 } }}>
    ...
    {community.length > 0 && <><Typography variant="h4" sx={{ fontWeight: 950, mt: 6, mb: 2.5 }}>Community templates</Typography><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2,1fr)", lg: "repeat(3,1fr)" }, gap: 2.5 }}>{community.filter(item => (category === "All" || item.category === category) && `${item.name} ${item.description}`.toLowerCase().includes(query.toLowerCase())).map(item => <Box key={item.id} sx={{ p: 3, borderRadius: 5, border: `1px solid ${theme.palette.divider}`, bgcolor: "background.paper" }}><Chip label={item.featured ? "Featured community" : item.category} size="small" color={item.featured ? "primary" : "default"} /><Typography variant="h6" sx={{ mt: 2, fontWeight: 950 }}>{item.name}</Typography><Typography sx={{ color: "text.secondary", minHeight: 52, mt: .75 }}>{item.description}</Typography><Typography variant="caption" sx={{ color: "text.secondary" }}>By {item.author_name || "Smart Manage creator"}</Typography><Stack direction="row" alignItems="center" gap={1} sx={{ my: 2 }}><Rating value={Number(item.rating)} readOnly size="small" /><Typography variant="caption">{item.review_count} reviews · {item.downloads} installs</Typography></Stack><Stack direction="row" gap={1}><Button fullWidth variant="contained" onClick={() => installTemplate(item.template_key, item.name)} disabled={Boolean(installing)} sx={{ borderRadius: 3, fontWeight: 900 }}>Use template</Button><Button variant="outlined" onClick={() => setReviewing(item)} sx={{ borderRadius: 3, fontWeight: 900, whiteSpace: "nowrap" }}>Rate & review</Button></Stack></Box>)}</Box></>}
    ...
    <Dialog open={Boolean(reviewing)} onClose={() => !savingReview && setReviewing(null)} fullWidth maxWidth="sm"><DialogTitle sx={{ fontWeight: 950 }}>Review {reviewing?.name}</DialogTitle><DialogContent sx={{ display: "grid", gap: 2, pt: "12px !important" }}><Box><Typography sx={{ fontWeight: 800, mb: 1 }}>Your rating</Typography><Rating value={reviewDraft.rating} onChange={(_, value) => setReviewDraft(v => ({ ...v, rating: value || 1 }))} size="large" /></Box><TextField label="Review" multiline minRows={4} value={reviewDraft.review} onChange={e => setReviewDraft(v => ({ ...v, review: e.target.value }))} placeholder="Share what worked well..." /></DialogContent><DialogActions sx={{ p: 3 }}><Button onClick={() => setReviewing(null)}>Cancel</Button><Button variant="contained" disabled={savingReview} onClick={saveReview}>{savingReview ? "Saving..." : "Save review"}</Button></DialogActions></Dialog>
  </Box>;
}
