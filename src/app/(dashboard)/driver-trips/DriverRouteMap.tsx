...
function geocodeCandidates(address:string):string[]{
  ...
  const spanishStreet=cleaned
    ...
    .replace(/\s+/g," ")
    .trim();
  return [...new Set([spanishStreet,cleaned].filter(Boolean))];
}

async function geocode(address?:string):Promise<Point|null>{
  if(!address)return null;
  for(const query of geocodeCandidates(address)){
    const params=new URLSearchParams({format:"jsonv2",limit:"1",countrycodes:query.toLowerCase().includes("spain")?"es":"",q:query});
