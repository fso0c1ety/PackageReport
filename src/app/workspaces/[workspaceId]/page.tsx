...
export default function WorkspacePage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && workspaceId) {
      ...
      fetch(`http://192.168.0.108:4000/api/workspaces/${workspaceId}`)
        .then(res => res.json())
        .then(ws => {
          if (ws && ws.id) {
  ...
  const handleDelete = async () => {
    if (!menuTableId) return;
    await fetch(`http://192.168.0.108:4000/api/tables/${menuTableId}`, {
      method: "DELETE",
    });
    handleMenuClose();
    setSelected(null);
  ...
  const fetchTables = async () => {
    setLoading(true);
    const res = await fetch(`http://192.168.0.108:4000/api/workspaces/${workspaceId}/tables`);
