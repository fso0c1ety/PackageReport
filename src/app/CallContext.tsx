...
type CallStatus = 'calling' | 'ringing' | 'connected' | 'declined' | 'missed' | 'ended' | 'busy' | 'error' | null;
...
function getMediaErrorMessage(error: unknown, requestedVideo: boolean) {
    ...
    if (name === 'NotReadableError' || name === 'TrackStartError') {
        return 'The camera or microphone is already being used by another application.';
    }
    return requestedVideo
        ? 'The video call could not access your camera or microphone.'
        : 'The audio call could not access your microphone.';
}
...
export const CallProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('broadcast', { event: 'call_offer' }, async ({ payload }) => {
                ...
                if (currentActiveCall || incomingCallRef.current) {
                    await sendSignal('call_busy', {
                        targetId: normalizedOffer?.callerId,
                        callId: normalizedOffer?.callId,
                    });
                    return;
                }
            ...
            .on('broadcast', { event: 'call_ringing' }, ({ payload }) => {
                if (payload?.targetId !== currentUser.id) return;
                if (activeCallRef.current?.callId && payload?.callId && activeCallRef.current.callId !== payload.callId) return;
            ...
            .on('broadcast', { event: 'call_reject' }, ({ payload }) => {
                if (payload?.targetId !== currentUser.id) return;
                finishCall(payload?.reason === 'missed' ? 'missed' : 'declined');
            })
    ...
    useEffect(() => {
        let cancelled = false;
        authenticatedFetch(getApiUrl('calls/ice-config'))
            .then(async (response) => response.ok ? response.json() : null)
            .then((configuration) => {
                if (!cancelled && Array.isArray(configuration?.iceServers) && configuration.iceServers.length > 0) {
                    iceConfigurationRef.current = { iceServers: configuration.iceServers };
                }
            })
            .catch(() => {
                // Public STUN fallback remains available for local/LAN calls.
            });
        return () => {
            cancelled = true;
        };
    ...
    const initializePeerConnection = (targetUserId: string) => {
        ...
        pc.onconnectionstatechange = () => {
            ...
            if (pc.connectionState === 'connected' && callStatusRef.current !== 'connected') {
                setCallStatus('connected');
                setConnectedAt(Date.now());
            }
            if (pc.connectionState === 'failed') {
                console.error("[WebRTC] P2P Connection failed. This usually means a TURN server is required for this network.");
                setCallError('The call could not connect. A TURN server may be required for this network.');
                finishCall('ended');
            }
    ...
    const finishCall = (
        reason: Exclude<CallStatus, 'calling' | 'ringing' | 'connected' | 'error' | null> = 'ended',
        writeLog = true,
    ) => {
        const callerData = activeCallRef.current;
        const currentStatus = callStatusRef.current;
        ...
        if (callerData?.isCaller && writeLog) {
            const callType = callerData.isVideo ? 'Video' : 'Audio';
            const logText = currentStatus === 'connected' || duration > 0
                ? `${callType} call ended — ${formatDuration(duration)}`
                : reason === 'declined'
                    ? `Declined ${callType.toLowerCase()} call`
                    : reason === 'busy'
                        ? `${callType} call — busy`
                        : `Missed ${callType.toLowerCase()} call`;

            void authenticatedFetch(getApiUrl(`chats/${callerData.userId}`), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ text: logText }),
            }).catch((err) => console.error('Failed to log call', err));

            // Prevent the legacy cleanup path from writing a second call event.
            activeCallRef.current = { ...callerData, isCaller: false };
        }

        if (reason === 'declined') setCallNotice('Call declined');
        if (reason === 'missed') setCallNotice('Call not answered');
        if (reason === 'busy') setCallNotice('User is busy on another call');
        if (reason === 'ended' && currentStatus === 'connected') {
            setCallNotice(`Call ended — ${formatDuration(duration)}`);
        }
    ...
    const startCall = async (targetId: string, isVideo: boolean, otherUser?: any) => {
        ...
        if (activeCallRef.current || incomingCallRef.current) {
            setCallNotice('You are already in another call');
            return;
        }
        if (!signalingReady) {
            setCallError('Calling service is still connecting. Please try again in a moment.');
            return;
        }
        try {
            const stream = await navigator.mediaDevices.getUserMedia({
                video: isVideo ? { facingMode: cameraFacingMode } : false,
                audio: true,
            });
            ...
            callTimeoutRef.current = setTimeout(() => {
                if (callStatusRef.current === 'calling' || callStatusRef.current === 'ringing') {
                    void sendSignal('call_end', { targetId, callId, reason: 'missed' });
                    finishCall('missed');
                }
            }, 45000);
        } catch (err) {
            console.error("Failed to start call", err);
            setCallError(getMediaErrorMessage(err, isVideo));
    ...
    const acceptCall = async () => {
        ...
        try {
            ...
            const stream = await navigator.mediaDevices.getUserMedia({
                video: normalizedIncomingCall.isVideo ? { facingMode: cameraFacingMode } : false,
                audio: true,
            });
            setLocalStream(stream);
            setActiveCall({
                ...
                callId: normalizedIncomingCall.callId,
            });
            ...
            setCallError(getMediaErrorMessage(err, !!incomingCall?.isVideo));
            setCallStatus('error');
            rejectCall();
            finishCall('ended', false);
        }
    ...
    const switchCamera = async () => {
        if (!activeCall?.isVideo || !localStream) return;
        ...
        try {
            ...
            const sender = peerConnection.current?.getSenders().find((candidate) => candidate.track?.kind === 'video');
            if (sender) {
                await sender.replaceTrack(replacementTrack);
            }

            const previousVideoTracks = localStream.getVideoTracks();
            const updatedStream = new MediaStream([...localStream.getAudioTracks(), replacementTrack]);
            setLocalStream(updatedStream);
            ...
            setCallError('The camera could not be switched on this device.');
        }
    };
    ...
    return (
        <CallContext.Provider value={{ startCall, showIncomingCall }}>
                        ...
                        {(!activeCall.isVideo || !hasRemoteVideo) && (
                            <Box sx={{
                                ...
                                {callStatus !== 'connected' ? (
                                    <Typography variant="caption" color={activeCall.isVideo ? 'rgba(255,255,255,0.78)' : 'text.secondary'}>{getCallStatusText()}</Typography>
                                ) : (
                    ...
                    <Box sx={{ p: 3, display: 'flex', justifyContent: 'center', gap: 3, flexWrap: 'wrap', bgcolor: activeCall.isVideo ? 'rgba(0,0,0,0.8)' : 'background.paper', borderTop: activeCall.isVideo ? 'none' : '1px solid', borderColor: 'divider' }}>
                        ...
                        {activeCall.isVideo && (
                            <Tooltip title="Switch camera">
                                <IconButton onClick={() => { void switchCamera(); }} sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: '#fff', p: 2, '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' } }}>
                                    <CameraswitchIcon />
                                </IconButton>
                            </Tooltip>
                        )}
            ...
            {activeCall && isCallMinimized && (
                <Box sx={{
                    ...
                    <Box sx={{ px: 1.5, pb: 1.5, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
                        ...
                        {activeCall.isVideo && (
                            <IconButton size="small" onClick={() => { void switchCamera(); }} sx={{ bgcolor: 'action.selected' }}>
            ...
            <Snackbar open={!!callError} autoHideDuration={6000} onClose={() => setCallError('')} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
                <Alert severity="error" variant="filled" onClose={() => setCallError('')}>
                    {callError}
                </Alert>
            </Snackbar>
            <Snackbar open={!!callNotice} autoHideDuration={3500} onClose={() => setCallNotice('')} message={callNotice} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }} />
