...
export default function MobileBottomNavigation() {
  ...
  const current = portalDestinations.find((item) => `${pathname}${typeof window !== "undefined" ? window.location.search : ""}`.startsWith(item.path))?.path || false;
  return (
    <Paper data-mobile-bottom-navigation="true" elevation={12} sx={{ display: { xs: "block", md: "none" }, position: "fixed", zIndex: 1300, left: 0, right: 0, bottom: 0, pb: "env(safe-area-inset-bottom)", borderRadius: 0, borderTop: "1px solid", borderColor: "divider" }}>
      ...
      >
        {portalDestinations.map((item) => <BottomNavigationAction key={`${item.label}-${item.path}`} label={item.label} value={item.path} icon={item.icon} />)}
      </BottomNavigation>
    </Paper>
  );
}
