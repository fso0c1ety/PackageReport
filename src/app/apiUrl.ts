...
export async function authenticatedFetch(url: string, options: AuthenticatedFetchOptions = {}) {
  ...
  try {
    ...
    if (!response && typeof window !== 'undefined' && isTransientNetworkError(err)) {
      try {
        const parsed = new URL(requestUrl);
        const isLocalApiFallbackCandidate =
          (parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1' || parsed.hostname === '::1') &&
          parsed.port === '4000' &&
          (parsed.pathname === '/api' || parsed.pathname.startsWith('/api/'));

        if (isLocalApiFallbackCandidate) {
          const browserOrigin = getBrowserOrigin();
          if (browserOrigin) {
            const fallbackUrl = `${browserOrigin}${parsed.pathname}${parsed.search}`;
        ...
        // Ignore URL parsing and fallback errors and continue to the existing fallbacks.
      }
    }
