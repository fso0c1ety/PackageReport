...
export default function DemoRequestForm() {
  ...
  return <Box component="form" onSubmit={submit} noValidate sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 2 }}>
    <TextField required label="Full Name" value={values.name} onChange={set("name")} inputProps={{ maxLength: 120 }} />
    <TextField required label="Company Name" value={values.companyName} onChange={set("companyName")} inputProps={{ maxLength: 160 }} />
    <TextField required type="email" label="Work Email" value={values.email} onChange={set("email")} inputProps={{ maxLength: 200 }} />
    <TextField label="Phone" value={values.phone} onChange={set("phone")} inputProps={{ maxLength: 60 }} />
    <TextField required select label="Business Type" value={values.businessType} onChange={set("businessType")}>
    ...
    <TextField select label="Team Size" value={values.teamSize} onChange={set("teamSize")}><MenuItem value="">Not specified</MenuItem>{TEAM_SIZES.map((size) => <MenuItem key={size} value={size}>{size}</MenuItem>)}</TextField>
    ...
    <TextField sx={{ gridColumn: "1/-1" }} multiline minRows={4} label="Message / Requirements" value={values.message} onChange={set("message")} inputProps={{ maxLength: 3000 }} />
