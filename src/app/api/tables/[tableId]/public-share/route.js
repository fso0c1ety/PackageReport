...
async function getManageableTable(tableId, userId) {
  const result = await pool.query(`
    SELECT t.id, t.name, t.workspace_id, t.public_share_token, t.public_share_enabled
...
export async function GET(req, { params }) {
  ...
  return NextResponse.json({ enabled: table.public_share_enabled, token: table.public_share_enabled ? table.public_share_token : null });
}
...
export async function POST(req, { params }) {
  ...
  await pool.query("UPDATE tables SET public_share_token=$1, public_share_enabled=TRUE WHERE id=$2", [token, tableId]);
