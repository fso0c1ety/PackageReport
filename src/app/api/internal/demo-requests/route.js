...
import { provisionDemoRequest, resetDemoWorkspace, sendDemoAccessEmail } from "../../_lib/demoProvisioning";
...
export async function GET(req) {
  ...
  const rows = (await pool.query(`SELECT dr.*,w.name workspace_name,w.demo_expires_at,u.email prospect_email
...
export async function POST(req) {
  ...
  try {
    ...
    if (action === "resend") {
      ...
      await sendDemoAccessEmail({ pool, req, request }); return NextResponse.json({ success: true });
    }
    if (action === "extend") {
      ...
      const row = (await pool.query(`UPDATE workspaces w SET demo_expires_at=GREATEST(COALESCE(w.demo_expires_at,NOW()),NOW())+($2||' days')::interval
        FROM demo_requests dr WHERE dr.id=$1 AND dr.demo_workspace_id=w.id AND w.is_demo=TRUE RETURNING w.id,w.demo_expires_at`, [id, days])).rows[0];
    ...
    if (action === "delete") {
      ...
      if (String(body.confirmDelete || "") !== request.company_name) return NextResponse.json({ error: "Company name confirmation is required" }, { status: 400 });
