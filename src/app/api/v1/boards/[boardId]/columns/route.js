...
export async function GET(req, { params }) {
  ...
  const result = await pool.query("SELECT columns FROM tables WHERE id=$1", [boardId]);
  return NextResponse.json({ data: Array.isArray(result.rows[0]?.columns) ? result.rows[0].columns : [] }, { headers: { "Cache-Control": "private, no-store" } });
}
