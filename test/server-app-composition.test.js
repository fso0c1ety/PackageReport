...
test("public auth and billing routes mount before the protected API boundary", () => {
  ...
  const routes = Object.fromEntries(
    ["auth", "billing", "users", "people", "automation", "emailer", "friends", "chats"].map((name) => [name, { name }]),
  );
  ...
  assert.deepEqual(mounted.slice(3).map((entry) => entry[1].name), ["users", "people", "automation", "emailer", "friends", "chats"]);
});
