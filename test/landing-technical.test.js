...
test("landing metadata, robots and sitemap use Smart Manage public identity", () => {
  ...
  assert.match(layout, /Smart Manage — One Workspace/); assert.match(layout, /openGraph/); assert.match(layout, /canonical/);
...
test("public contact endpoint validates, rate limits and blocks basic bots", () => {
  ...
  assert.match(route, /recent.length >= 5/); assert.match(route, /body.website/); assert.match(route, /startedAt/); assert.match(route, /sendEmail/);
  const page = fs.readFileSync(require.resolve("../src/app/page.tsx"), "utf8");
  assert.match(page, /smartmanage:analytics/); assert.match(page, /fetch\("\/api\/contact"/); assert.doesNotMatch(page, /emailjs\.send/);
});
