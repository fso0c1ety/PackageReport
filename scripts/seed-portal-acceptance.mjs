...
export async function seedPortalAcceptance({connectionString=process.env.DATABASE_URL,password=process.env[PASSWORD_ENV],env=process.env}={}) {
  ...
  try { await client.query("BEGIN"); const ids={}; for (const key of Object.keys(accounts)) ids[key]=await ensureAccount(client,key,password);
    ...
    const groupNames=["Explorers","Sunflowers"],childNames=["Klea Morina","Noel Gashi"];
    const groups=await link("daycare","Groups",["teacherA","teacherB"],(id,account,row,table,i)=>({[col(table,"Name")?.id]:groupNames[i],[col(table,"Educator")?.id]:person(id,account),_classTeacherUserId:id})); await link("daycare","Parents",["parentA","parentB"],(id)=>({_linkedUserId:id})); const children=await link("daycare","Children",["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,_classTeacherUserId:ids[i===0?"teacherA":"teacherB"],[col(table,"Full Name")?.id]:childNames[i],[col(table,"Group")?.id]:[{tableId:groups.table.id,rowId:groups.records[i]?.id,label:groupNames[i],tableName:"Groups"}]})); for(const name of ["Attendance","Documents","Payments"]) await link("daycare",name,["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,_classTeacherUserId:ids[i===0?"teacherA":"teacherB"],[col(table,"Child")?.id]:[{tableId:children.table.id,rowId:children.records[i]?.id,label:childNames[i],tableName:"Children"}]})); await link("daycare","Meals",["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,educatorUserId:ids[i===0?"teacherA":"teacherB"],[col(table,"Assigned Groups")?.id]:[{tableId:groups.table.id,rowId:groups.records[i]?.id,label:groupNames[i],tableName:"Groups"}]})); await link("daycare","Activities",["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,_classTeacherUserId:ids[i===0?"teacherA":"teacherB"],[col(table,"Group")?.id]:[{tableId:groups.table.id,rowId:groups.records[i]?.id,label:groupNames[i],tableName:"Groups"}],[col(table,"Children")?.id]:[{tableId:children.table.id,rowId:children.records[i]?.id,label:childNames[i],tableName:"Children"}]}));
    const patientNames=["Elira Krasniqi","Arben Hoxha"];
    const patients=await link("dental","Patients",["patientA","patientB"],(id,_,row,table,i)=>({_linkedUserId:id,_linkedPatientUserId:id,_assignedDoctorUserId:ids[i===0?"doctorA":"doctorB"],[col(table,"Name")?.id]:patientNames[i]})); for(const name of ["Appointments","Treatments"]) await link("dental",name,["doctorA","doctorB"],(id,account,row,table,i)=>({[col(table,"Dentist")?.id]:person(id,account),[col(table,"Patient")?.id]:[{tableId:patients.table.id,rowId:patients.records[i]?.id,label:patientNames[i],tableName:"Patients"}],_assignedDoctorUserId:id,_linkedPatientUserId:ids[i===0?"patientA":"patientB"]})); for(const name of ["Lab Requests","Documents","Dental Billing"]) await link("dental",name,["patientA","patientB"],(id,_,row,table,i)=>({[col(table,"Patient")?.id]:[{tableId:patients.table.id,rowId:patients.records[i]?.id,label:patientNames[i],tableName:"Patients"}],_linkedPatientUserId:id,_assignedDoctorUserId:ids[i===0?"doctorA":"doctorB"],...(name==="Documents"?{[col(table,"Visibility")?.id]:"Shared",[col(table,"Status")?.id]:"Shared"}:{})}));
