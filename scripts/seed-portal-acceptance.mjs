...
const roles = {
  driverA:["fleet","driver","member","driver"],driverB:["fleet","driver","member","driver"],manager:["fleet","dispatcher","manager","dispatcher"],
...
const scopeFor = (portal) => portal === "driver" ? {scope:"assigned_to_me",field:"_assignedDriverUserId"} : portal === "teacher" ? {scope:"assigned_to_me",field:"classTeacherUserId"} : portal === "doctor" ? {scope:"assigned_to_me",field:"assignedDoctorUserId"} : portal === "client" ? {scope:"my_company",field:"clientCompanyId"} : {scope:"custom",rule:{field:portal === "parent" ? "linkedParentUserId" : "linkedPatientUserId",value:"$current_user"}};
...
export async function seedPortalAcceptance({connectionString=process.env.DATABASE_URL,password=process.env[PASSWORD_ENV],env=process.env}={}) {
  ...
  try { await client.query("BEGIN"); const ids={}; for (const key of Object.keys(accounts)) ids[key]=await ensureAccount(client,key,password);
    ...
    await link("daycare","Groups",["teacherA","teacherB"],(id,account,row,table)=>({[col(table,"Educator")?.id]:person(id,account)})); await link("daycare","Parents",["parentA","parentB"],(id)=>({_linkedUserId:id})); await link("daycare","Children",["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,_classTeacherUserId:ids[i===0?"teacherA":"teacherB"]}));
    await link("dental","Patients",["patientA","patientB"],(id)=>({_linkedUserId:id,_linkedPatientUserId:id})); for(const name of ["Appointments","Treatments"]) await link("dental",name,["doctorA","doctorB"],(id,account,row,table,i)=>({[col(table,"Dentist")?.id]:person(id,account),_assignedDoctorUserId:id,_linkedPatientUserId:ids[i===0?"patientA":"patientB"]}));
