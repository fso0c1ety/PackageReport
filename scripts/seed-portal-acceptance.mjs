...
export async function seedPortalAcceptance({connectionString=process.env.DATABASE_URL,password=process.env[PASSWORD_ENV],env=process.env}={}) {
  ...
  try { await client.query("BEGIN"); const ids={}; for (const key of Object.keys(accounts)) ids[key]=await ensureAccount(client,key,password);
    ...
    await link("daycare","Groups",["teacherA","teacherB"],(id,account,row,table)=>({[col(table,"Educator")?.id]:person(id,account),_classTeacherUserId:id})); await link("daycare","Parents",["parentA","parentB"],(id)=>({_linkedUserId:id})); await link("daycare","Children",["parentA","parentB"],(id,_,row,table,i)=>({_linkedParentUserId:id,_classTeacherUserId:ids[i===0?"teacherA":"teacherB"]})); await link("daycare","Documents",["parentA","parentB"],(id)=>({_linkedParentUserId:id}));
